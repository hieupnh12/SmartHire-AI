...
export function TestForm({ test, jobs, busy, onSave, onDirty }: {
  test?: JobTest; jobs: { id: number; title: string }[]; busy: boolean;
  ...
  const { register, handleSubmit, reset, formState: { errors, isDirty } } = useForm<Form>({
    resolver: zodResolver(schema), defaultValues: { title: test?.title ?? "", jobId: test?.jobId ?? 0,
  ...
  return <form className="space-y-4" onSubmit={handleSubmit(async values => {
    ...
    <fieldset disabled={busy} className="space-y-4">
      ...
      <label className="block space-y-1 text-sm">Vị trí tuyển dụng
