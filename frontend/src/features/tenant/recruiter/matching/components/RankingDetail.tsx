  ...
  return <dialog ref={dialog} onCancel={(event) => { event.preventDefault(); onClose(); }} aria-labelledby="ranking-detail-title" aria-modal="true" className="fixed inset-0 m-auto h-dvh w-full max-w-6xl overflow-y-auto border-0 bg-[var(--color-surface)] p-0 text-[var(--color-on-surface)] shadow-2xl backdrop:bg-slate-900/45 backdrop:backdrop-blur-sm sm:h-[calc(100dvh-32px)] sm:w-[calc(100%-32px)] sm:rounded-2xl sm:border sm:border-[var(--color-border-default)]">
    ...
    <div className="space-y-6 p-4 sm:p-6">
      <section className="relative overflow-hidden rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-card)] p-5 shadow-sm sm:p-6">
        ...
        <nav className="relative mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3" aria-label="Thao tác với ứng viên"><Link className={`${button} bg-[var(--color-surface-card)]`} to={`/recruiter/assessments?applicationId=${row.applicationId}`}><Code2 className="size-4 text-[var(--color-primary)]" aria-hidden="true" />Xem Assessment</Link><Link className={`${button} bg-[var(--color-surface-card)]`} to={`/recruiter/ai-interviews?applicationId=${row.applicationId}`}><MessageSquareText className="size-4 text-[var(--color-primary)]" aria-hidden="true" />Xem AI Interview</Link><Link className={primary} to={`/recruiter/interviews?applicationId=${row.applicationId}`}><CalendarDays className="size-4" aria-hidden="true" />Đặt lịch Interview</Link></nav>
      </section>
