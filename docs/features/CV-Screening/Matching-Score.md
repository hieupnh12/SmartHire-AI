...
- Trang Sàng lọc CV: Gemini (hoặc heuristic nếu chưa có key) đọc file CV của đúng job, so với `job_skills`/JD, ghi `match_scores` với `verdict` + điểm fit. Rank-v1 35/15/30/20 chỉ dùng ở Matching khi đã có assessment/interview.
